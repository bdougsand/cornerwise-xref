...
def body_output(req, s, status=200, extra_headers={}, content_type="text/plain"):
...
def lambda_response(body, status=200,
...
def azure(fn):
    ...
    try:
        ...
        if isinstance(response, str):
            body_output(req, status, headers, response)
    ...
    def do_run(event, context):
        ...
        if isinstance(response, tuple):
            if len(response) == 2:
                ...
                status, headers, response = response
        if isinstance(response, str):
            return lambda_response(response, status, headers)
